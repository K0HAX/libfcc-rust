...
impl From<libfcc_data::OperatorClass> for mysql::Value {
	...
	match operator_class {
	    ...
	    libfcc_data::OperatorClass::AmateurExtra => mysql::Value::Bytes("E".as_bytes().to_vec()),
	    ...
	    libfcc_data::OperatorClass::TechnicianPlus => mysql::Value::Bytes("P".as_bytes().to_vec()),
...
impl From<libfcc_data::EntityType> for mysql::Value {
	...
	match val {
	    libfcc_data::EntityType::TransfereeContact => mysql::Value::Bytes("CE".as_bytes().to_vec()),
	    libfcc_data::EntityType::LicenseeContact => mysql::Value::Bytes("CL".as_bytes().to_vec()),
	    libfcc_data::EntityType::AssignorOrTransferorContact => mysql::Value::Bytes("CR".as_bytes().to_vec()),
	    ...
	    libfcc_data::EntityType::LicenseeOrAssignee => mysql::Value::Bytes("L".as_bytes().to_vec()),
	    libfcc_data::EntityType::Owner => mysql::Value::Bytes("O".as_bytes().to_vec()),
	    libfcc_data::EntityType::AssignorOrTransferor => mysql::Value::Bytes("R".as_bytes().to_vec()),
...
impl From<libfcc_data::ApplicantTypeCode> for mysql::Value {
	...
	match val {
	    libfcc_data::ApplicantTypeCode::AmateurClub => mysql::Value::Bytes("B".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::Corporation => mysql::Value::Bytes("C".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::GeneralPartnership => mysql::Value::Bytes("D".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::LimitedPartnership => mysql::Value::Bytes("E".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::LimitedLiabilityPartnership => mysql::Value::Bytes("F".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::GovernmentalEntity => mysql::Value::Bytes("G".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::Other => mysql::Value::Bytes("H".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::Individual => mysql::Value::Bytes("I".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::JointVenture => mysql::Value::Bytes("J".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::LimitedLiabilityCompany => mysql::Value::Bytes("L".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::MilitaryRecreation => mysql::Value::Bytes("M".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::Consortium => mysql::Value::Bytes("O".as_bytes().to_vec()),
	    libfcc_data::ApplicantTypeCode::Partnership => mysql::Value::Bytes("P".as_bytes().to_vec()),
	    ...
	    libfcc_data::ApplicantTypeCode::UnincorporatedAssociation => mysql::Value::Bytes("U".as_bytes().to_vec()),
...
impl From<libfcc_data::EnStatusCode> for mysql::Value {
	...
	match val {
	    libfcc_data::EnStatusCode::TerminationPending => mysql::Value::Bytes("X".as_bytes().to_vec()),
...
impl From<libfcc_data::LicenseStatus> for mysql::Value {
	...
	match val {
	    ...
	    libfcc_data::LicenseStatus::PendingLegalStatus => mysql::Value::Bytes("L".as_bytes().to_vec()),
	    libfcc_data::LicenseStatus::ParentStationCanceled => mysql::Value::Bytes("P".as_bytes().to_vec()),
...
impl From<libfcc_data::DevelopmentalStaDemonstration> for mysql::Value {
	...
	match val {
	    libfcc_data::DevelopmentalStaDemonstration::Developmental => mysql::Value::Bytes("D".as_bytes().to_vec()),
	    libfcc_data::DevelopmentalStaDemonstration::Demonstration => mysql::Value::Bytes("M".as_bytes().to_vec()),
	    libfcc_data::DevelopmentalStaDemonstration::Regular => mysql::Value::Bytes("N".as_bytes().to_vec()),
	    libfcc_data::DevelopmentalStaDemonstration::SpecialTemporaryAuthority => mysql::Value::Bytes("S".as_bytes().to_vec()),
	    libfcc_data::DevelopmentalStaDemonstration::Unknown => mysql::Value::Bytes("".as_bytes().to_vec()),
	}
    }
}
// End Enums
...
fn split_am_rows(input_records: Vec<libfcc_data::Amateur>) -> Vec<Vec<libfcc_data::Amateur>> {
	...
	if (n % split_modulus) == 0 {
	    ...
	    this_vec.push(am_row.clone());
	    retval.push(this_vec.clone());
	    i = retval.len() - 1;
	} else {
	    retval[i].push(am_row.clone());
	}
	n = n+1;
    }
    return retval;
...
fn insert_am_rows_batch(mut conn: mysql::PooledConn, am_records: Vec<libfcc_data::Amateur>, chunk_id: u32, tot_chunks: usize) {
    let mut tx = conn.start_transaction(TxOpts::default()).unwrap();
    let result = tx.exec_batch("INSERT INTO ham_AM (\
				`Record Type`, \
...
pub fn insert_am_rows(sql_url: &str, am_records: Vec<libfcc_data::Amateur>) {
	...
	for am_chunk in am_records_split {
	    ...
	    s.spawn(move |_| {
		insert_am_rows_batch(conn, am_chunk, this_chunk.clone(), tot_chunks);
	    });
	}
    });
...
// BEGIN ham_EN //
fn insert_en_rows_batch(mut conn: PooledConn, en_records: Vec<libfcc_data::Entity>, chunk_id: u32, tot_chunks: usize) {
    let mut tx = conn.start_transaction(TxOpts::default()).unwrap();
    let result = tx.exec_batch("INSERT INTO ham_EN (\
				`Record Type`, \
				`Unique System Identifier`, \
				`ULS File Number`, \
				`EBF Number`, \
...
fn split_en_rows(input_records: Vec<libfcc_data::Entity>) -> Vec<Vec<libfcc_data::Entity>> {
	...
	if (n % split_modulus) == 0 {
	    ...
	    this_vec.push(en_row.clone());
	    retval.push(this_vec.clone());
	    i = retval.len() - 1;
	} else {
	    retval[i].push(en_row.clone());
	}
	n = n+1;
    }
    return retval;
...
pub fn insert_en_rows(sql_url: &str, en_records: Vec<libfcc_data::Entity>) {
	...
	for en_chunk in en_records_split {
	    ...
	    println!("Starting chunk {}/{}", this_chunk, tot_chunks);
	    s.spawn(move|_| {
		insert_en_rows_batch(conn, en_chunk, this_chunk.clone(), tot_chunks);
	    });
	}
    });
    println!("In Place Scope exited");
...
// BEGIN ham_HD //
fn insert_hd_rows_batch(mut conn: PooledConn, hd_records: Vec<libfcc_data::ApplicationLicenseHeader>, chunk_id: u32, tot_chunks: usize) {
    let mut tx = conn.start_transaction(TxOpts::default()).unwrap();
    let result = tx.exec_batch("INSERT INTO ham_HD (\
				`Record Type`, \
				`Unique System Identifier`, \
				`ULS File Number`, \
				`EBF Number`, \
...
fn split_hd_rows(input_records: Vec<libfcc_data::ApplicationLicenseHeader>) -> Vec<Vec<libfcc_data::ApplicationLicenseHeader>> {
	...
	if (n % split_modulus) == 0 {
	    ...
	    this_vec.push(hd_row.clone());
	    retval.push(this_vec.clone());
	    i = retval.len() - 1;
	} else {
	    retval[i].push(hd_row.clone());
	}
	n = n+1;
    }
    return retval;
...
pub fn insert_hd_rows(sql_url: &str, hd_records: Vec<libfcc_data::ApplicationLicenseHeader>) {
	...
	for hd_chunk in hd_records_split {
	    ...
	    println!("Starting chunk {}/{}", this_chunk, tot_chunks);
	    s.spawn(move|_| {
		insert_hd_rows_batch(conn, hd_chunk, this_chunk.clone(), tot_chunks);
	    });
	}
    });
    println!("In Place Scope exited");
